...
# }}}
# {{{ compose generation and script helpers

# Detect the compose command available on this system.
# Sets COMPOSE_COMMAND to the first working option, or returns 1.
detect_compose_command() {
    if [ -n "${MORLOC_CONTAINER_ENGINE:-}" ]; then
        _engine="$MORLOC_CONTAINER_ENGINE"
    else
        _engine="$CONTAINER_ENGINE"
    fi
    # Try plugin-style first (docker compose / podman compose)
    if $_engine compose version >/dev/null 2>&1; then
        COMPOSE_COMMAND="$_engine compose"
        return 0
    fi
...
# Generate docker-compose.yml in MORLOC_DATA_HOME
# Usage: generate_compose_file
generate_compose_file() {
    compose_dir="$MORLOC_DATA_HOME"
...
# DO NOT EDIT — generated by morloc-manager, changes will be overwritten.
# For customization, create docker-compose.override.yml in this directory.
services:
  morloc:
    image: \${MORLOC_IMAGE}
    shm_size: "${SHARED_MEMORY_SIZE}"
    environment:
      - HOME=\${MORLOC_HOST_HOME}
    working_dir: \${MORLOC_HOST_HOME}/work
    volumes:
      - \${MORLOC_HOST_HOME}/\${MORLOC_INSTALL_DIR}/\${MORLOC_VERSION}:\${MORLOC_HOST_HOME}/${MORLOC_DATA_RELDIR}${_z}
      - \${MORLOC_WORK_DIR}:\${MORLOC_HOST_HOME}/work${_z}
  ...
  morloc-dev:
    ...
    environment:
      - HOME=\${MORLOC_HOST_HOME}
      - PATH=/root/.ghcup/bin:\${MORLOC_HOST_HOME}/.local/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
    working_dir: \${MORLOC_HOST_HOME}/work
    volumes:
      - \${MORLOC_HOST_HOME}/\${MORLOC_INSTALL_DIR}/local:\${MORLOC_HOST_HOME}/${MORLOC_DATA_RELDIR}${_z}
      - \${MORLOC_HOST_HOME}/\${MORLOC_INSTALL_DIR}/local/home/.local/bin:\${MORLOC_HOST_HOME}/${MORLOC_BIN_BASENAME}${_z}
      - \${MORLOC_HOST_HOME}/\${MORLOC_INSTALL_DIR}/local/home/.stack:\${MORLOC_HOST_HOME}/.stack${_z}
      - \${MORLOC_WORK_DIR}:\${MORLOC_HOST_HOME}/work${_z}
COMPOSE_EOF

    if [ $? -ne 0 ]; then
        print_error "Failed to write $compose_file"
        return 1
    fi
...
generate_menv_script() {
    _script_path="$1"
    _compose_dir="$MORLOC_DATA_HOME"

    print_info "Generating menv at $_script_path"

    cat > "$_script_path" << 'MENV_EOF'
#!/usr/bin/env sh
# Morloc container entry point — generated by morloc-manager
# Usage: menv [--dev] [--shell] [-x EXTRA]... [--] [COMMAND...]

COMPOSE_DIR="${MORLOC_HOME:-${HOME}/.local/share/morloc}"
SERVICE="morloc"
SHELL_MODE=""
EXTRA=""
...
# Read engine from .env if not already set
if [ -z "${MORLOC_CONTAINER_ENGINE:-}" ] && [ -f "$COMPOSE_DIR/.env" ]; then
    MORLOC_CONTAINER_ENGINE=$(grep '^MORLOC_CONTAINER_ENGINE=' "$COMPOSE_DIR/.env" | cut -d= -f2-)
fi
...
if [ "$SERVICE" = "morloc-dev" ]; then
    _install_dir=$(grep '^MORLOC_INSTALL_DIR=' "$COMPOSE_DIR/.env" 2>/dev/null | cut -d= -f2-)
    _install_dir="${_install_dir:-.local/share/morloc/versions}"
    mkdir -p "$HOME/$_install_dir/local/home/.local/bin"
    mkdir -p "$HOME/$_install_dir/local/home/.stack"
fi

if [ -n "$SHELL_MODE" ]; then
    exec $COMPOSE -f "$COMPOSE_DIR/docker-compose.yml" --env-file "$COMPOSE_DIR/.env" \
        run --rm $EXTRA "$SERVICE" /bin/bash
else
    exec $COMPOSE -f "$COMPOSE_DIR/docker-compose.yml" --env-file "$COMPOSE_DIR/.env" \
        run --rm -T $EXTRA "$SERVICE" "$@"
fi
MENV_EOF
...
Generates a docker-compose.yml, .env, and a single ${BOLD}${GREEN}menv${RESET} wrapper script
...
cmd_install() {
    ...
    if ! detect_compose_command; then
        print_error "No 'docker compose' or compatible command found"
        print_info "Install Docker Compose plugin, or standalone docker-compose / podman-compose"
        exit 1
    fi
    print_info "Using compose command: $COMPOSE_COMMAND"

    # Generate compose file, .env, menv script, and compatibility shims
    generate_compose_file
        ...
        case "$1" in
            ...
            -a|--all)
                ...
                if [ -f "$MORLOC_DATA_HOME/docker-compose.override.yml" ]; then
                    print_warning "User override file preserved: $MORLOC_DATA_HOME/docker-compose.override.yml"
                fi
...
cmd_info() {
    ...
    install_dir="$HOME/${MORLOC_INSTALL_DIR}"
    env_file="$MORLOC_DATA_HOME/.env"
    compose_file="$MORLOC_DATA_HOME/docker-compose.yml"
    override_file="$MORLOC_DATA_HOME/docker-compose.override.yml"
    versions=""
    if [ -d "$install_dir" ]; then
        for d in "$install_dir"/*/; do
            [ -d "$d" ] || continue
    ...
    if [ -f "$compose_file" ]; then
        printf "Compose: %s %s[exists]%s\n" "$compose_file" "$GREEN" "$RESET"
    else
        printf "Compose: %s %s[missing]%s\n" "$compose_file" "$RED" "$RESET"
    fi
    if [ -f "$env_file" ]; then
    ...
    if [ -f "$override_file" ]; then
        printf "Override: %s %s[exists]%s\n" "$override_file" "$CYAN" "$RESET"
    else
        printf "Override: (create %s for custom config)\n" "$override_file"
    fi
