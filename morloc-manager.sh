...
MORLOC_DATA="\${MORLOC_HOME:-\${HOME}/.local/share/morloc}"
. "\$MORLOC_DATA/.env"
...
if [ "\$SERVICE" = "morloc-dev" ]; then
    _install_dir="\${MORLOC_INSTALL_DIR:-.local/share/morloc/versions}"
    mkdir -p "\$HOME/\$_install_dir/local/home/.local/bin"
    mkdir -p "\$HOME/\$_install_dir/local/home/.stack"
fi
...
cmd_install() {
    ...
    mkdir -p "$HOME/${MORLOC_INSTALL_DIR}/${LOCAL_VERSION}/home/.local/bin"
    mkdir -p "$HOME/${MORLOC_INSTALL_DIR}/${LOCAL_VERSION}/home/.stack"
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            -a|--all)
                ...
                rm -f "$MORLOC_DATA_HOME/.env"
                rm -f "$MORLOC_DATA_HOME/morloc.flags"
                # Remove legacy compose files if present
                rm -f "$MORLOC_DATA_HOME/docker-compose.yml"
                if [ -f "$MORLOC_DATA_HOME/docker-compose.override.yml" ]; then
                ...
                if [ -d "$MORLOC_DEPENDENCY_DIR" ]; then
                    rm -f "$MORLOC_DEPENDENCY_DIR"/*.flags
                fi
...
cmd_select() {
    ...
    then
        ...
        sed -i "s|^MORLOC_VERSION=.*|MORLOC_VERSION=${version}|" "$env_file"
        sed -i "s|^MORLOC_IMAGE=.*|MORLOC_IMAGE=${CONTAINER_BASE_FULL}:${version}|" "$env_file"
    else
...
update_environment() {
  ...
  if [ "$update_usr" = "true" ]; then
      ...
      sed -i "s|^MORLOC_IMAGE=.*|MORLOC_IMAGE=${user_container}|" "$env_file"
  ...
  if [ "$update_dev" = "true" ]; then
      ...
      sed -i "s|^MORLOC_DEV_IMAGE=.*|MORLOC_DEV_IMAGE=${dev_container}|" "$env_file"
  ...
  if [ -f "$flags_file" ]; then
      sed -i "s|^MORLOC_ENV_FLAGS=.*|MORLOC_ENV_FLAGS=${flags_file}|" "$env_file"
      print_info "Activated environment flags: $flags_file"
  else
      sed -i "s|^MORLOC_ENV_FLAGS=.*|MORLOC_ENV_FLAGS=|" "$env_file"
  fi
...
reset_environment() {
  ...
  if [ "$reset_update_dev" = "true" ]; then
      sed -i "s|^MORLOC_DEV_IMAGE=.*|MORLOC_DEV_IMAGE=${CONTAINER_BASE_TEST}:latest|" "$env_file"
