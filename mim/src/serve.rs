...
/// What a frozen artifact declared it serves, or `None` when it declared
/// nothing. A manifest predating the recorded exposure also yields `None`: such
/// an artifact cannot say what it meant to serve, and guessing from the
/// installed set would publish whatever happened to be built alongside.
fn spec_from_exposure(m_manifest: Option<&FreezeManifest>) -> Option<crate::ServeSpec> {
    let ex = &m_manifest?.exposure;
    if ex.is_empty() {
        return None;
    }
...
pub fn build_serve_image(
    ...
    // Which optional state directories the artifact actually carries. A frozen
    // environment always brings its runtime, lock and programs (see
    // `freeze::frozen_paths`); the rest travel only if the environment had them,
    // and a COPY of a path the archive lacks fails the build.
    let optional_state: Vec<String> = ["fdb", "src", "modules"]
        .iter()
        .filter(|d| {
            let p = context_dir.join(d);
            p.is_dir() && fs::read_dir(&p).map(|mut e| e.next().is_some()).unwrap_or(false)
        })
        .map(|d| d.to_string())
        .collect();
    // The image serves what the environment declared, through the same command
    // builder the manager uses to launch a serve, so a frozen artifact and a live
    // `mim start` cannot disagree about what a module answers on.
    //
    // The bind is 0.0.0.0 because a container's loopback is its own: a published
    // port never reaches a service bound to 127.0.0.1 inside. That makes the
    // nexus fail closed without a token, which is the posture wanted here -- the
    // image refuses to serve an unauthenticated non-loopback endpoint until the
    // operator supplies MORLOC_MCP_TOKEN or overrides the command deliberately.
    let cmd = match spec_from_exposure(m_manifest.as_ref()) {
        Some(spec) => crate::build_router_command(
            CONTAINER_MORLOC_STATE,
            DEPLOY_HTTP_PORT,
            "0.0.0.0",
            &spec,
            false,
    ...
    let dockerfile_content = crate::dockerfile::generate_deploy_dockerfile(
    ...
    validate_programs(engine, tag, programs, Vec::new(), Vec::new(), verbose)?;
...
/// The image a deployment image is built on: the environment's own image, which
/// carries pixi to install the toolchain, the activation wrapper every process
/// goes through, and the morloc compiler a sandboxed eval forks.
///
/// There is no substitute for it. A generic base would be missing all three, and
/// an artifact built on one would fail at the first `RUN`, so an absent image is
/// reported here rather than papered over -- the deployment image is built where
/// the environment lives, or the environment image is loaded first.
fn resolve_base_from_manifest(
    ...
    let Some(fm) = m_manifest else {
        return Err(ManagerError::UnfreezeError(
            "the frozen artifact has no freeze-manifest.json beside its tarball, so the \
             image it was frozen from is unknown. Re-freeze the environment, or name the \
             image explicitly with --base."
                .to_string(),
        ));
    };

    // A user-supplied environment recipe can be rebuilt from the Dockerfile the
    // freeze carried, which is the one case where the image need not already
    // exist on this machine.
    if let Some(fel) = &fm.env_layer {
        if let Some(tag) = &fel.image_tag {
            if image_exists_locally(engine, tag) {
                return Ok(tag.clone());
            }
        }
        return Ok(rebuild_env_image(engine, &fm.base_image, fm, fel));
    }

    if image_exists_locally(engine, &fm.base_image) {
        return Ok(fm.base_image.clone());
    }
    Err(ManagerError::UnfreezeError(format!(
        "the environment image '{}' this artifact was frozen from is not on this machine. \
         A deployment image is that image with the environment's runtime and toolchain \
         baked in, so it cannot be built without it.\n  \
         Build the deployment image where the environment lives, load the environment \
         image here first, or name a replacement with --base.",
        fm.base_image
    )))
}
...
mod tests {
    ...
    fn manifest_with(exposure: ExposureConfig) -> FreezeManifest {
        FreezeManifest {
            morloc_version: Version::new(0, 101, 0),
    ...
    fn a_deployment_image_serves_the_declared_set() {
        ...
        let spec = spec_from_exposure(Some(&manifest_with(ex))).expect("a spec");
    ...
    #[test]
    fn an_artifact_that_declared_nothing_gets_no_default_command() {
        assert!(spec_from_exposure(Some(&manifest_with(ExposureConfig::default()))).is_none());
        // A manifest predating the recorded exposure cannot say what it meant to
        // serve; guessing would publish whatever was installed beside it.
        assert!(spec_from_exposure(None).is_none());
    }
