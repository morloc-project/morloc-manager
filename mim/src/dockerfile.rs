...
/// In-image path of the activation wrapper this generator writes. Every
/// container process goes through it: it exports CONDA_PREFIX, runs pixi's
/// shell-hook for PATH, and sources the conda `activate.d` scripts that supply
/// $CC/$AR and each language's runtime environment.
pub const ACTIVATE_WRAPPER: &str = "/usr/local/bin/morloc-activate";

/// Inputs to the deployment-image generator. See [`generate_deploy_dockerfile`].
pub struct DeployDockerfileInput<'a> {
    /// The environment's own image. It carries pixi (to install the toolchain),
    /// the activation wrapper, and the morloc compiler that a sandboxed eval
    /// forks, so a deployment image is that image with the environment's
    /// mounted halves baked in rather than a fresh base.
    pub base_image: &'a str,
    /// The image's default command: the router invocation that serves the
    /// environment's exposed set, built by the same function the manager uses to
    /// launch a serve. Empty when the environment exposed nothing, which leaves
    /// the image a command line with no default command.
    pub cmd: &'a [String],
    /// Optional state directories present in the frozen artifact
    /// (`fdb`, `src`, `modules`), in the order they should be copied.
    pub optional_state: &'a [String],
    /// Port the router listens on inside the image.
    pub http_port: u16,
    /// Image labels, as `(key, value)`, describing what was frozen. An image
    /// that has left the manager's world has nothing tracking it, so what it
    /// holds has to travel inside it.
    pub labels: &'a [(String, String)],
    /// Emit a HEALTHCHECK. Podman's OCI output format drops the instruction and
    /// warns, so it is Docker-only.
    pub healthcheck: bool,
}

/// Inputs to the slim deployment-image generator. See
/// [`generate_slim_dockerfile`].
pub struct SlimDockerfileInput<'a> {
    /// The environment's own image, used only as the first stage: it has pixi
    /// and the lock reproduces the toolchain in it, which is then cut down.
    pub env_image: &'a str,
    /// The base the environment's image was built on. The final stage starts
    /// from it, so the system packages the environment apt-installed have the
    /// same names and the C library is the one the programs were validated
    /// against.
    pub base_image: &'a str,
    /// The environment's container-only system packages, installed again in
    /// the final stage.
    pub extras: &'a BuildExtras,
    /// Build-context-relative corporate CA PEM, as for the environment image.
    pub cert_file: Option<&'a str>,
    pub cmd: &'a [String],
    pub optional_state: &'a [String],
    pub http_port: u16,
...
/// Render the Dockerfile that turns a frozen environment into a self-contained
/// deployment image. Deterministic for a given input.
///
/// An environment is an image plus three host-side pieces, and this bakes the
/// two the image lacks. The runtime (MORLOC_HOME) is copied because it was
/// built after the image was: the nexus, libmorloc, the language bindings and
/// every installed program's launcher live there. The toolchain is *installed*
/// rather than copied, from the lock the artifact carries -- the solved prefix
...
/// command serves the router while `docker run <image> <program> <args>` runs
/// the same programs through their generated command line.
///
/// The default command comes from the environment's exposed set, so the image
/// serves what its author declared rather than everything that happened to be
/// installed alongside it.
pub fn generate_deploy_dockerfile(input: &DeployDockerfileInput) -> String {
    let mh = crate::serve::CONTAINER_MORLOC_HOME;
    let state = crate::serve::CONTAINER_MORLOC_STATE;
    let env_dir = crate::serve::CONTAINER_PIXI_DIR;
    let mut out = String::new();
    out.push_str("# Auto-generated by mim; do not edit\n");
    out.push_str(&format!("FROM {}\n\n", input.base_image));

    // Ordered by how often each step's inputs change, cheapest to invalidate
    // last. Solving the toolchain is minutes and its inputs move rarely, so it
    // goes first; the programs are seconds and move every build, so they go
    // last. Reversing any of this makes an ordinary rebuild reinstall conda.
    out.push_str("# The toolchain, reinstalled from the lock the artifact carries\n");
    out.push_str(&format!("COPY pixi/ {env_dir}/\n"));
    out.push_str(&format!(
        "RUN {} install --locked --manifest-path {env_dir}/pixi.toml\n\n",
        crate::serve::CONTAINER_PIXI_BIN
    ));

    // MORLOC_HOME/bin holds the nexus and every installed program's launcher.
    // The base image does not have it on PATH -- in a pliable environment it is
    // a mount that only exists at run time -- so a deployment image that skipped
    // this would have no `morloc-nexus` to run.
    out.push_str("# The runtime's binaries: the nexus, and a launcher per program\n");
    out.push_str(&format!("ENV PATH=\"{mh}/bin:${{PATH}}\"\n"));
    ...
    // sandboxed eval builds under it, so a dangling one fails them at the point
    // of first use. World-writable because the image does not know what user it
    // will be run as.
    out.push_str(&writable_state_block());

    out.push_str("# The runtime, built into the environment after its image was\n");
    out.push_str(&format!("COPY runtime/ {mh}/\n\n"));

    out.push_str("# Installed programs and the module sources behind them\n");
    out.push_str(&format!("COPY exe/ {state}/exe/\n"));
    for dir in input.optional_state {
        out.push_str(&format!("COPY {dir}/ {state}/{dir}/\n"));
    }
    out.push('\n');
    ...
    if !input.labels.is_empty() {
        out.push_str("# What this image holds, for whoever meets it without its environment\n");
    ...
    out.push_str("# Serve by default; any other command runs against the same programs\n");
    out.push_str(&format!("ENTRYPOINT [\"{ACTIVATE_WRAPPER}\"]\n"));
    let quoted: Vec<String> = input.cmd.iter().map(|a| format!("\"{a}\"")).collect();
    out.push_str(&format!("CMD [{}]\n", quoted.join(", ")));
    out
}

/// The two directories under the state root a running program writes to. The
/// nexus caches under $HOME and the router logs each daemon's startup under
/// `<state>/logs`; nothing mounts either in a deployment image, so both are
/// created world-writable, because the image cannot know what user runs it.
/// HOME is baked as well: the base image only symlinks it into the state root,
/// and an engine gives a user with no passwd entry `HOME=/`.
fn writable_state_block() -> String {
    let state = crate::serve::CONTAINER_MORLOC_STATE;
    let home = crate::serve::CONTAINER_HOME;
    format!(
        "# A writable HOME and log directory, both under the state root\n\
         RUN mkdir -p {state}/home {state}/logs && chmod 1777 {state}/home {state}/logs\n\
         ENV HOME={home}\n\n"
    )
}

/// Build-context file listing the prefix-relative paths the slim image
/// deletes, NUL-separated, and its companion naming the removed records.
pub const PRUNE_FILES: &str = "prune.txt";
pub const PRUNE_RECORDS: &str = "prune-records.txt";

/// Render the Dockerfile for a slim deployment image: the programs and what
/// runs them, without what built them.
///
/// Two stages. The first is the environment's image with the toolchain
/// reinstalled from the lock, exactly as the full deployment image begins, and
/// then cut: the manager has computed, from the environment's own package
/// records, the files of every package that exists to build things and of
/// whatever only those packages pulled in. The stage checks that each record
/// it is about to delete is present -- the list was computed against the same
/// lock, so a missing record means the list does not describe this prefix --
/// and deletes the files. The second stage starts from the base the
/// environment was built on and copies in the survivors, the runtime, and the
/// programs. Bytes deleted in the first stage never reach the second, which
/// is why the cut is a stage rather than a `RUN rm` in the full image.
///
/// The final stage has no pixi, so the activation wrapper does not call it:
/// PATH is baked, and the wrapper exports the prefix and sources whatever
/// `activate.d` scripts survived. The compiler activation scripts belong to
/// packages the cut removed. Everything else -- the paths, the entrypoint, the
/// default command -- is as in the full image, for the same reasons.
pub fn generate_slim_dockerfile(input: &SlimDockerfileInput) -> String {
    let mh = crate::serve::CONTAINER_MORLOC_HOME;
    let state = crate::serve::CONTAINER_MORLOC_STATE;
    let env_dir = crate::serve::CONTAINER_PIXI_DIR;
    let prefix = format!("{env_dir}/.pixi/envs/default");
    let mut out = String::new();
    out.push_str("# Auto-generated by mim; do not edit\n");
    out.push_str("# Stage 1: the toolchain, reproduced from the lock, then cut down to what runs\n");
    out.push_str(&format!("FROM {} AS toolchain\n", input.env_image));
    out.push_str(&format!("COPY pixi/ {env_dir}/\n"));
    out.push_str(&format!(
        "RUN {} install --locked --manifest-path {env_dir}/pixi.toml\n",
        crate::serve::CONTAINER_PIXI_BIN
    ));
    out.push_str(&format!("COPY {PRUNE_RECORDS} {PRUNE_FILES} /tmp/\n"));
    out.push_str(&format!(
        "RUN cd {prefix} \\\n \
         && while IFS= read -r rec; do test -f \"conda-meta/$rec\" \\\n      \
         || {{ echo \"conda-meta/$rec is not installed; the prune list does not describe this prefix\" >&2; exit 1; }}; \\\n    \
         done < /tmp/{PRUNE_RECORDS} \\\n \
         && xargs -0 -r rm -f < /tmp/{PRUNE_FILES}\n\n"
    ));

    out.push_str("# Stage 2: the base the environment was built on, carrying only what runs\n");
    out.push_str(&format!("FROM {}\n", input.base_image));
    out.push_str("ENV DEBIAN_FRONTEND=noninteractive\n\n");
    if let Some(cert_file) = input.cert_file {
        out.push_str("# Corporate CA bundle (trusted by the programs' own network calls)\n");
        out.push_str(&format!(
            "COPY {cert_file} /usr/local/share/ca-certificates/morloc-corp.crt\n\n"
        ));
    }
    out.push_str(&apt_block(input.extras, input.cert_file.is_some(), false));
    ...
    out.push_str("# Env-owned home (symlink into the state root)\n");
    out.push_str(&format!(
        "RUN mkdir -p /home && ln -sfn {state}/home {}\n\n",
    ...
    out.push_str("# The cut-down toolchain, at the path the programs were linked against\n");
    out.push_str(&format!("COPY --from=toolchain {env_dir}/ {env_dir}/\n\n"));
    out.push_str("# The runtime, without its headers and the source it was built from\n");
    out.push_str(&format!("COPY runtime/ {mh}/\n\n"));
    out.push_str("# Installed programs and the module sources behind them\n");
    out.push_str(&format!("COPY exe/ {state}/exe/\n"));
    for dir in input.optional_state {
    ...
    out.push_str("# No pixi in this image: PATH is baked rather than produced by a shell hook\n");
    out.push_str(&format!(
        "ENV PATH=\"{mh}/bin:{}:{}\"\n",
        crate::serve::CONTAINER_PIXI_ENV_BIN,
        crate::serve::CONTAINER_PATH_TAIL
    ));
    out.push_str(&format!("ENV MORLOC_HOME={mh}\n"));
    out.push_str(&format!("ENV MORLOC_STATE={state}\n"));
    out.push_str(&format!("ENV CONDA_PREFIX={prefix}\n"));
    out.push_str("ENV LANG=C.UTF-8 LC_ALL=C.UTF-8\n\n");
    ...
    out.push_str("# Env identity + activation of what survived, for every container process\n");
    out.push_str(&activation_script(
        &identity_block(false),
        &crate::serve::slim_activate_lines(),
    ));
    out.push_str(&format!("ENTRYPOINT [\"{ACTIVATE_WRAPPER}\"]\n"));
    if input.cmd.is_empty() {
        return out;
...
/// Make a label value safe inside the double-quoted form. Label values are
/// free text -- a module synopsis, a list of program names -- and an unescaped
/// quote or newline in one would end the instruction early and produce a
/// Dockerfile that fails to parse, or worse, parses as something else.
...
/// The apt step every requirement-derived image starts with: the tools the
/// build needs before a conda environment exists, the identity shim, and the
/// container-only system packages. Shared by the environment image and the
/// slim deployment image, which starts from the same base and needs the same
/// system packages under the programs it carries.
fn apt_block(extras: &BuildExtras, cert: bool, dev: bool) -> String {
    let mut out = String::new();
    ...
    for pkg in &extras.system_packages {
...
/// The lines of the activation wrapper that establish who the process is.
/// See [`generate_dockerfile`] for why the two flavours differ.
fn identity_block(dev: bool) -> Vec<String> {
    if dev {
...
/// The `RUN` that writes the activation wrapper: `#!/bin/bash`, the identity
/// lines, the activation lines, `exec "$@"`. Lines are emitted through a
/// single-quoted `printf`, so none may contain a single quote.
fn activation_script(identity: &[String], activation: &[String]) -> String {
    let script_lines: String = identity
        .iter()
        .chain(activation.iter())
        .map(|l| format!(" '{l}'"))
        .collect();
    format!(
        "RUN printf '%s\\n' '#!/bin/bash'{script_lines} 'exec \"$@\"' > {ACTIVATE_WRAPPER} && chmod +x {ACTIVATE_WRAPPER}\n"
    )
}
...
pub fn generate_dockerfile(input: &DockerfileInput) -> String {
    ...
    out.push_str(&apt_block(input.extras, input.cert_file.is_some(), input.dev));
    ...
    out.push_str(&activation_script(
        &identity_block(input.dev),
        &crate::serve::conda_activate_lines(),
    ));
    out.push_str(&format!("ENTRYPOINT [\"{ACTIVATE_WRAPPER}\"]\n"));
...
";
    ...
    fn deploy(cmd: &[&str], optional: &[&str], healthcheck: bool) -> String {
        let cmd: Vec<String> = cmd.iter().map(|s| s.to_string()).collect();
        let optional: Vec<String> = optional.iter().map(|s| s.to_string()).collect();
        generate_deploy_dockerfile(&DeployDockerfileInput {
            base_image: "localhost/morloc-env:latest",
            cmd: &cmd,
            optional_state: &optional,
            http_port: 8080,
            healthcheck,
            labels: &[("morloc.environment".to_string(), "dev".to_string())],
        })
    }

    /// The router invocation a served environment produces, abbreviated.
    const SERVE_CMD: [&str; 4] = ["morloc-nexus", "router", "--mcp", "dna"];

    #[test]
    fn a_deployment_image_carries_the_runtime_and_installs_the_toolchain() {
        // The two halves an environment keeps outside its image. Without the
        // first there is no nexus to run; without the second no interpreter to
        // run a pool.
        let df = deploy(&SERVE_CMD, &[], false);
        assert!(df.contains("COPY runtime/ /opt/morloc/"), "{df}");
        assert!(df.contains("COPY pixi/ /env/"), "{df}");
        assert!(df.contains("install --locked --manifest-path /env/pixi.toml"), "{df}");
    }

    #[test]
    fn a_deployment_image_puts_the_runtime_binaries_on_path() {
        // In a pliable environment MORLOC_HOME is a mount that exists only at run
        // time, so the base image has no reason to put it on PATH. A deployment
        // image bakes it, and must say so or it has no `morloc-nexus`.
        let df = deploy(&SERVE_CMD, &[], false);
        assert!(df.contains("ENV PATH=\"/opt/morloc/bin:${PATH}\""), "{df}");
        assert!(df.contains("ENV MORLOC_STATE=/opt/morloc-state"), "{df}");
    }

    #[test]
    fn a_deployment_image_serves_by_default_and_stays_a_command_line() {
        // One image, both interfaces: the entrypoint activates the toolchain and
        // nothing else, so overriding the command runs a program's own CLI.
        let df = deploy(&SERVE_CMD, &[], false);
        assert!(df.contains(&format!("ENTRYPOINT [\"{ACTIVATE_WRAPPER}\"]")), "{df}");
        assert!(
            df.contains("CMD [\"morloc-nexus\", \"router\", \"--mcp\", \"dna\"]"),
            "{df}"
        );
    }
    ...
    fn an_environment_with_no_views_yields_a_command_line_image() {
        ...
        // It is still a complete image: runtime, toolchain and programs.
        assert!(df.contains("COPY runtime/ /opt/morloc/"), "{df}");
        assert!(df.contains("COPY exe/ /opt/morloc-state/exe/"), "{df}");
    }

    #[test]
    fn optional_state_is_copied_only_when_the_artifact_has_it() {
        // A COPY of a path the archive lacks fails the build, so the generator
        // must be told what actually travelled.
        let bare = deploy(&SERVE_CMD, &[], false);
        assert!(!bare.contains("COPY fdb/"), "{bare}");
        assert!(bare.contains("COPY exe/ /opt/morloc-state/exe/"), "{bare}");

        let full = deploy(&SERVE_CMD, &["fdb", "modules"], false);
    ...
    #[test]
    fn expensive_layers_come_before_volatile_ones() {
        // Solving the toolchain is minutes and its inputs move rarely; the
        // programs are seconds and move every build. Emitting them the other way
        // round makes an ordinary rebuild reinstall conda for nothing.
        let df = deploy(&SERVE_CMD, &["fdb"], false);
        let at = |needle: &str| df.find(needle).unwrap_or_else(|| panic!("missing: {needle}\n{df}"));
        assert!(at("COPY pixi/") < at("install --locked"));
        assert!(at("install --locked") < at("COPY runtime/"));
        assert!(at("COPY runtime/") < at("COPY exe/"));
        assert!(at("COPY exe/") < at("COPY fdb/"));
        // Metadata that changes with the exposed set comes last of all.
        assert!(at("COPY fdb/") < at("CMD ["));
    }

    #[test]
    fn a_label_value_cannot_break_out_of_its_instruction() {
        // Label values are free text: a module synopsis, a list of names. An
        // unescaped quote would end the instruction early and produce a
        // Dockerfile that fails to parse, or parses as something else.
        let labels = [(
            "morloc.note".to_string(),
            "a \"quoted\" thing\nand a newline".to_string(),
        )];
        let df = generate_deploy_dockerfile(&DeployDockerfileInput {
            base_image: "img:1",
            cmd: &[],
            optional_state: &[],
            http_port: 8080,
        ...
        let line = df
            .lines()
            .find(|l| l.starts_with("LABEL morloc.note="))
            .expect("the label line");
        assert_eq!(line, r#"LABEL morloc.note="a \"quoted\" thing and a newline""#);
    }

    fn slim(cmd: &[&str], cert: Option<&str>, base: &str) -> String {
        let cmd: Vec<String> = cmd.iter().map(|s| s.to_string()).collect();
        let extras = BuildExtras { system_packages: vec!["libgl1".to_string()] };
        generate_slim_dockerfile(&SlimDockerfileInput {
            env_image: "localhost/morloc-env:latest",
            base_image: base,
            extras: &extras,
            cert_file: cert,
            cmd: &cmd,
            optional_state: &["src".to_string()],
            http_port: 8080,
            labels: &[("morloc.flavor".to_string(), "slim".to_string())],
            healthcheck: false,
        })
    }

    /// The cut happens in a stage the final image does not inherit, so the
    /// deleted bytes are gone rather than hidden under a later layer.
    #[test]
    fn a_slim_image_cuts_in_one_stage_and_copies_survivors_into_another() {
        let df = slim(&SERVE_CMD, None, "docker.io/library/debian:bookworm-slim");
        let at = |needle: &str| df.find(needle).unwrap_or_else(|| panic!("missing: {needle}\n{df}"));
        assert!(df.starts_with("# Auto-generated by mim; do not edit\n# Stage 1"), "{df}");
        assert!(df.contains("FROM localhost/morloc-env:latest AS toolchain"), "{df}");
        assert!(at("install --locked") < at("xargs -0 -r rm -f"));
        assert!(at("xargs -0 -r rm -f") < at("FROM docker.io/library/debian:bookworm-slim"));
        assert!(df.contains("COPY --from=toolchain /env/ /env/"), "{df}");
        assert!(!df.contains("/opt/morloc-runtime"), "{df}");
        assert!(!df.contains("pixi.sh/install.sh"), "{df}");
    }

    /// A record the list names but the prefix lacks means the list was
    /// computed against a different solve; deleting on it would be guessing.
    #[test]
    fn a_slim_image_checks_the_prune_list_against_the_installed_records() {
        let df = slim(&SERVE_CMD, None, "debian:bookworm-slim");
        let at = |needle: &str| df.find(needle).unwrap_or_else(|| panic!("missing: {needle}\n{df}"));
        assert!(df.contains(&format!("COPY {PRUNE_RECORDS} {PRUNE_FILES} /tmp/")), "{df}");
        assert!(df.contains("test -f \"conda-meta/$rec\""), "{df}");
        assert!(at("test -f \"conda-meta/$rec\"") < at("xargs -0 -r rm -f"));
    }

    /// The layout the pools were compiled against, unchanged; the system
    /// packages the environment declared, installed again; no pixi, so PATH
    /// is a fact of the image rather than a shell hook's output.
    #[test]
    fn a_slim_image_keeps_the_layout_and_bakes_the_environment() {
        let df = slim(&SERVE_CMD, None, "debian:bookworm-slim");
        assert!(df.contains("COPY runtime/ /opt/morloc/"), "{df}");
        assert!(df.contains("COPY exe/ /opt/morloc-state/exe/"), "{df}");
        assert!(df.contains("COPY src/ /opt/morloc-state/src/"), "{df}");
        assert!(df.contains("ENV PATH=\"/opt/morloc/bin:/env/.pixi/envs/default/bin:"), "{df}");
        assert!(df.contains("ENV CONDA_PREFIX=/env/.pixi/envs/default"), "{df}");
        assert!(df.contains("ENV HOME=/home/morloc"), "{df}");
        assert!(df.contains("chmod 1777 /opt/morloc-state/home /opt/morloc-state/logs"), "{df}");
        assert!(df.contains("libnss-wrapper libgl1"), "{df}");
        assert!(!df.contains("shell-hook"), "{df}");
        assert!(df.contains("activate.d"), "{df}");
        assert!(df.contains("CMD [\"morloc-nexus\", \"router\", \"--mcp\", \"dna\"]"), "{df}");
        assert!(!slim(&[], None, "debian:bookworm-slim").contains("CMD ["));
    }

    #[test]
    fn a_slim_image_trusts_the_corporate_ca_like_the_environment() {
        let df = slim(&SERVE_CMD, Some("certs/corp.pem"), "debian:bookworm-slim");
        assert!(df.contains("COPY certs/corp.pem /usr/local/share/ca-certificates/morloc-corp.crt"), "{df}");
        assert!(df.contains("update-ca-certificates"), "{df}");
        assert!(df.contains("SSL_CERT_FILE="), "{df}");
        assert!(!slim(&SERVE_CMD, None, "debian:bookworm-slim").contains("update-ca-certificates"));
    }

    #[test]
    fn a_slim_image_on_ubuntu_reclaims_the_base_user() {
        assert!(slim(&SERVE_CMD, None, "docker.io/library/ubuntu:24.04").contains("userdel -r ubuntu"));
        assert!(!slim(&SERVE_CMD, None, "debian:bookworm-slim").contains("userdel"));
    }

    /// Both deployment images bake HOME: an engine gives a user with no
    /// passwd entry `HOME=/`, and the nexus cache lands wherever HOME points.
    #[test]
    fn deployment_images_bake_home_and_a_writable_log_dir() {
        let df = deploy(&SERVE_CMD, &[], false);
        assert!(df.contains("ENV HOME=/home/morloc"), "{df}");
        assert!(df.contains("/opt/morloc-state/logs"), "{df}");
    }

    #[test]
    fn the_healthcheck_is_docker_only() {
        assert!(deploy(&SERVE_CMD, &[], true).contains("HEALTHCHECK"));
        assert!(!deploy(&SERVE_CMD, &[], false).contains("HEALTHCHECK"));
    }
